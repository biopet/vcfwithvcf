...
## Introduction
This tool combines the INFO fields of two VCF files.

## Example
To run this tool:
```bash
java -jar <VcfWithVcf-jar> -I input.vcf -s secondary.vcf -R reference.fa -o output.vcf
```

To open the help:
```bash
java -jar <VcfWithVcf-jar> --help
Usage: VcfWithVcf [options]
 ...
 ## Output
 This tool outputs a vcf file with the combined INFO fields.
